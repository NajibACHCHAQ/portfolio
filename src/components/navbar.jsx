...
const links = [
  ...
  { url: "/portfolio", title: "Réalisations" },
    ...
    <div className="navbar w-[100%] h-full flex items-center justify-between px-4 sm:px-8 md:px-12 lg:px-20 xl:px-48 text-xl">
      ...
      <div className="w-[fit-content] hidden md:flex gap-4">
        {links.map((link, index) => (
          <div className="hover:text-yellow-500" key={index}>
            <NavLink link={link} />
          </div>
        ))}
      </div>
      ...
      <div className="md:hidden lg:flex xl:w-1/3 xl:justify-center items-center">
        ...
        <Link href="/" className="text-sm bg-blue-500 rounded-md p-2 font-semibold flex items-center justify-center h-[fit-content]">
      ...
      <div className="hidden md:flex gap-4 w-[fit-content] float-right">
        <Link href="https://github.com/NajibACHCHAQ" target="blank">
          <Image src="/github.png" alt="" width={24} height={24} />
        </Link>
        <Link href="https://www.linkedin.com/in/najib-achchaq-589a651a8/" target="blank">
      ...
      <div className="md:hidden">
        ...
        {/* MENU LIST */}
        {open && (
          <motion.div variants={listVariants} initial="closed" animate="opened" className="absolute top-0 left-0 w-screen h-screen bg-blue-500 bg-opacity-80 text-white flex flex-col items-center justify-center gap-8 text-4xl z-40">
            {links.map((link, index) => (
